...
This is the aurdino board. Here we only consider a small part (the circled part which is the chip)which will be designed as a part of Physical design work.
The block diagram of the aurdino board is as shown below.
When we open up the IC of the processor it will look as shown below, which is known as package (eg: QFN-48).

The way chip is connected to the package via wire bounds is as shown below,

Inner view of the chip is as shown below,

The chip has many components within it. Few of them are as follows,
Pads: With the help of pads, the signal can be communicated from outside to inside and vice versa.
Core: This is the place where the digital logic sits.
Die: Die is the entire chip size.
                                           
Foundry IP’s: Foundry is the place where the chips actually get manufactured. These are used to communicate with components within the chip.
Macros: These are nothing but the digital blocks
